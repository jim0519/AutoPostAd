...
namespace Common
{
    ...
    public class CommonFunction
    {
        ...
        public static T ConvertBinaryToObject<T>(byte[] bData) where T : new()
        {
            //TextReader reader = null;
            try
            {
                using (var stream = new MemoryStream(bData))
                {
                    ...
                    //reader = System.IO.File.ReadAllText(bData, Encoding.UTF8);
                    //var fileContents = reader.ReadToEnd();
                //return JsonConvert.DeserializeObject<T>(fileContents);
            }
            catch (Exception ex)
        ...
        public static byte[] ConvertObjectToBinary<T>(T obj) where T : new()
        {
            try
            {
